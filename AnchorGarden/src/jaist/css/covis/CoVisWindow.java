...
import jp.ac.jaist.css.common.piccolo.BUtil;
import jp.ac.jaist.css.common.piccolo.MouseWheelRotationListener;
 ...
 *
 */
public class CoVisWindow extends RootWindow implements MouseWheelRotationListener {
...
//	public static final boolean LOGGINGSRC = true;
	...
	public CoVisWindow(CoVisBuffer b){
		super(b);
		BasicConfigurator.configure(); // Log4J Initialize
		// オープニングの鳴き声
		Informer.playSound("Rooster.wav");
		log = Logger.getLogger(CoVisWindow.class.getName());
		initializeMenu();
		switchBuffer(buffer);
	}
...
//		layoutPMR.add(pmc_src);
		...
		pmc_reset.addPActionListener(new PActionListener() {
			public void actionPerformed(PInputEvent e) {
				CoVisWindow.this.resetWorld();
			}
		});
		pmc_layout.addPActionListener(new PActionListener() {
			public void actionPerformed(PInputEvent e) {
				CoVisWindow.this.fitAndZoomHome(200);
			}
		});
