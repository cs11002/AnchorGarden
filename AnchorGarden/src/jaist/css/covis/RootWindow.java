...
import jp.ac.jaist.css.common.piccolo.MyPCanvas;
import jp.ac.jaist.css.common.piccolo.MyPFrame;
import jp.ac.jaist.css.common.piccolo.PrintPrintable;
...
public class RootWindow implements ActionListener, WindowStateListener {
	...
	public void constructFrame(){
		frame = new MyPFrame(CoVisWindow.WINDOWTITLE+" "+String.valueOf(windowid), false, null);
		RootWindow.windowid++;
		windows.add(this);
	...
	protected void initializeMenu(){
		...
		printPDF.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				...
				((CoVisWindow)RootWindow.this).pmenubar.mybar.setVisible(false);
				printPDF(canvas, "A4L", "ag2.pdf");
				openPDF("ag2.pdf");
				((CoVisWindow)RootWindow.this).pmenubar.mybar.setVisible(true);
			}
	...
	public void switchBuffer(CoVisBuffer b){
		...
		buffer.removeShowingWindow((CoVisWindow)this);
		...
		canvas.setMouseWheelRotationListener((CoVisWindow)this);
		...
		frame.setTitle(CoVisWindow.WINDOWTITLE+" ("+buffer.name+")"); //ウィンドウタイトルバー更新
		((CoVisWindow)this).pmenubar.manualFit(cam.getBoundsReference()); //右上の黄色いメニューをどうしてもぴったり合わせたい
		...
		buffer.addShowingWindow((CoVisWindow)this);
	...
	public void newFrame(){
		new CoVisWindow((CoVisBuffer)this.buffer);
	}
	...
	public void newBuffer(){
		Thread t = new Thread(){
			public void run(){
				...
				new CoVisWindow(new CoVisBuffer(sprop));				
			}
		};
		t.start();
	}
	...
	public void showSrcWin(boolean visible){
		if (srcWindow == null){
			srcWindow = new SrcWindow(buffer, (CoVisWindow)RootWindow.this);
