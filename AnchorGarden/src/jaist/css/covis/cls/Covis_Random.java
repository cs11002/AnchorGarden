...
public class Covis_Random extends Covis_Object {
	...
	public void init(boolean isAuto) {
		...
		if (!isAuto) {
			RandomConstructorDialog dialog = RandomConstructorDialog
					.showDialog(buffer.getWindow().frame, this,
							"Constructor of Random", "new Random( a , b );");
		...
		Class<?> cla;
		try {
			cla = Class.forName("jaist.css.covis.cls.Covis_String");
			Constructor<?> con = cla.getConstructor(CoVisBuffer.class,
					boolean.class);
			c_type = (Covis_Type) con.newInstance(buffer, true);
...
class RandomConstructorDialog extends JDialog implements KeyListener {
	...
	public RandomConstructorDialog(JFrame p, Covis_Random random, String title,
			String mes1) {
		super(p, title, true);
		parent = p;
	...
	public static RandomConstructorDialog showDialog(JFrame parent,
			Covis_Random random, String title, String mes1) {
		RandomConstructorDialog d = new RandomConstructorDialog(parent, random,
				title, mes1);
		d.setVisible(true);
