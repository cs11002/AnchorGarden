...
public class DateChooser extends JDialog implements ActionListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 3213453525851611533L;
	...
	JComboBox hourCombo, minCombo, secCombo;
	...
	public DateChooser (JFrame owner, GregorianCalendar d) {
		...
		for(int i=0;i<59;i++){
			secCombo.addItem(String.format("%02d", i));
		}
		timeP.add(secCombo);
		contentPane.add(timeP, BorderLayout.SOUTH);
		
		JButton nowB = new JButton("now time");
		nowB.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				...
				hourCombo.setSelectedIndex(h);
				minCombo.setSelectedIndex(m);
				secCombo.setSelectedIndex(s);
			}
		});
		timeP.add(nowB);
	...
	public void actionPerformed (ActionEvent evt) {
		...
		if (label.equals("<")) {
			...
			try {
				...
				h = hourCombo.getSelectedIndex();
				min = minCombo.getSelectedIndex();
				s = secCombo.getSelectedIndex();
			}
			catch (NumberFormatException e) {
				System.err.println(e.toString());
			}
